...
from helper import break_line, convert_bytes_to_readable_unit
from constants import MAX_CHAR_LENGTH_PER_LINE, MAX_ROW_COUNT


def print_directory_with_restricted_access(permission_issue):
    print("\n")
    if(len(permission_issue) > 0):
        print('\033[33m'+tabulate([[break_line(str(item), MAX_CHAR_LENGTH_PER_LINE)] for item in permission_issue], headers=["Index",
              "Could not Scan because of limited permission"], showindex="always", tablefmt="fancy_grid"))


def print_biggest_files(scanned, limit=10):
